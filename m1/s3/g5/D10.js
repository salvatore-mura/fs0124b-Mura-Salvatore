...
console.log(splitMe("hello i'm Tore"))
...
function onlyLetters(stringa) {
  return stringa.split("").filter(char => isNaN(parseInt(char))).join("")
}
...
console.log(isThisAnEmail('tore.291196@gmail.com'))
...
function whatDayIsIt() {
  let giorno = new Date();
  return giorno.toLocaleDateString('it-IT', { weekday: 'long' });
}
...
let mioCompleanno = new Date("2024-29-11")
...
function newestMovie() {
  return movies.reduce((film1, film2) => {
    let dataFilm1 = film1.Year;
    let dataFilm2 = film2.Year;
    if (dataFilm2 > dataFilm1) {
      return film2;
    } else {
      return film1;
    }
...
function sumAllTheYears() {
  return movies.reduce((total, movie) => {
    let releaseYear = parseInt(movie.Year)
